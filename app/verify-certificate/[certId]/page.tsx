...
async function getCertificate(certId: string) {
  if (!/^ILM-CERT-[23456789ABCDEFGHJKLMNPQRSTUVWXYZ]{8}$/.test(certId)) return null;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md w-full glass-card rounded-3xl p-10 text-center space-y-5">
          ...
          <h1 className="font-display text-2xl font-bold text-[var(--text-primary)]">Invalid Certificate ID</h1>
          <p className="text-sm text-[var(--text-muted)]">
            The ID{" "}
            <code className="font-mono text-xs bg-[var(--bg-secondary)] px-2 py-0.5 rounded">{certId}</code>{" "}
            is not valid. IDs follow the pattern{" "}
            <code className="font-mono text-xs bg-[var(--bg-secondary)] px-2 py-0.5 rounded">ILM-CERT-XXXXXXXX</code>.
          </p>
          <Link href="/" className="btn-secondary text-sm inline-flex items-center gap-2">← Return to Home</Link>
        </div>
      </div>
    );
  }
        ...
        <div className="max-w-lg w-full space-y-6">
          <div className="glass-card rounded-3xl p-10 border border-red-500/20 bg-red-500/5 text-center space-y-5">
            ...
            <div>
              ...
              <h1 className="font-display text-2xl font-bold text-[var(--text-primary)]">Certificate Revoked</h1>
            </div>
            <div className="text-left bg-[var(--bg-card)] rounded-xl p-4 space-y-3 text-sm">
              ...
              {cert.revokedAt && (
                <div className="flex justify-between">
                  ...
                  <span className="text-red-400">{fmt(cert.revokedAt)}</span>
                </div>
              )}
          ...
          <div className="text-center">
            <Link href="/" className="text-xs text-[var(--text-muted)] hover:text-[var(--accent)] transition-colors">← Return to Ilm Platform</Link>
          </div>
        </div>
      </div>
    );
        ...
        <div className="glass-card rounded-3xl p-8 border border-emerald-500/20 bg-emerald-500/5 space-y-6">
          ...
          <div className="text-center space-y-3">
            ...
            <div>
              ...
              <h1 className="font-display text-2xl font-bold text-[var(--text-primary)]">Valid Certificate</h1>
          ...
          <div className="bg-[var(--bg-card)] rounded-2xl p-5 space-y-4">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-[var(--text-primary)] font-medium">{fmt(cert.completionDate ?? cert.issuedAt)}</span>
                </div>
                <div className="flex justify-between text-xs mt-1">
                  <span className="text-[var(--text-muted)]">Issued</span>
                  <span className="text-[var(--text-primary)] font-medium">{fmt(cert.issuedAt)}</span>
                </div>
              </div>
            </div>
            <div className="h-px bg-[var(--border)]" />
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider font-semibold">Certificate ID</span>
              </div>
