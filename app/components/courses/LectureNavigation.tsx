...
import { CourseCompletionModal } from "../courses/CourseCompletionModal";
...
interface LectureNavigationProps {
  ...
  /** Used only to seed "recommended courses" in the completion modal */
    ...
    <div className="sticky top-0 z-30 bg-[var(--bg-secondary)] border-b border-[var(--border)] px-4 sm:px-6">
      <div className="flex items-center gap-3 h-12">
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {prevSlug ? (
            <Link
              href={`/courses/${courseSlug}/learn/${prevSlug}`}
          ...
          {nextSlug ? (
            <Link
              href={`/courses/${courseSlug}/learn/${nextSlug}`}
              className="p-1.5 text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card)] rounded-lg transition-colors"
              title="Next lesson"
...
export function LectureBottomBar(props: LectureNavigationProps) {
  ...
  const {
    lectureId, courseId, courseSlug, courseTitle, categoryId,
    prevSlug, prevTitle,
    nextSlug, nextTitle,
  ...
  const handleNext = async () => {
    ...
    if (nextSlug) router.push(`/courses/${courseSlug}/learn/${nextSlug}`);
    else if (isLastLecture) setShowCompletion(true);
  };
