...
export async function PUT(req: NextRequest) {
  try {
    ...
    await prisma.auditLog.create({ data: { userId: user.id, action: data.onboardingCompleted ? "ONBOARDING_COMPLETED" : "PROFILE_UPDATED", entityType: "LearnerProfile", entityId: profile.id } }); return successResponse(profile);
