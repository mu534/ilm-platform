...
export class ScholarApplicationService {
  static async saveDraft(userId: string, input: ScholarApplicationInput) {
    ...
    const data = { ...input, city: input.city || null, education: input.education || null, teachingExperience: input.teachingExperience || null, teachingYears: input.teachingYears ?? null };
    if (existing) return prisma.scholarApplication.update({ where: { id: existing.id }, data: { ...data, status: "DRAFT", decisionReason: null, reviewedAt: null, reviewedById: null } });
  ...
  static async review(actorId: string, applicationId: string, action: "UNDER_REVIEW" | "APPROVE" | "REJECT", notes?: string, reason?: string) {
    ...
    if (action === "UNDER_REVIEW") return prisma.scholarApplication.update({ where: { id: applicationId }, data: { status: "UNDER_REVIEW", reviewedById: actorId, reviewedAt: new Date(), internalNotes: notes || null } });
    if (action === "REJECT") {
      const result = await prisma.scholarApplication.update({ where: { id: applicationId }, data: { status: "REJECTED", reviewedById: actorId, reviewedAt: new Date(), internalNotes: notes || null, decisionReason: reason } });
    ...
    const result = await prisma.$transaction(async (tx) => {
      const updated = await tx.scholarApplication.update({ where: { id: applicationId }, data: { status: "APPROVED", reviewedById: actorId, reviewedAt: new Date(), internalNotes: notes || null, decisionReason: null } });
