...
import {
  ...
  FiEdit2, FiSearch, FiCheckCircle, FiBarChart2,
} from "react-icons/fi";
...
export default function AdminScholarsPage() {
  ...
  const fetchScholars = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) { setScholars(data.data); setFiltered(data.data); }
    } catch (err) { console.error(err); }
    finally { setLoading(false); }
  }, []);

  useEffect(() => { void fetchScholars(); }, [fetchScholars]);
  ...
  const remove = async (id: string, name: string) => {
    if (!confirm(`Delete scholar profile for "${name}"? This cannot be undone.`)) return;
    ...
    <div className="p-6 sm:p-8">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <h1 className="font-display text-2xl sm:text-3xl font-bold text-[var(--text-primary)]">Scholars</h1>
          <p className="text-[var(--text-muted)] text-sm mt-1">
            {scholars.length} total · {scholars.filter((s) => s.verified).length} verified
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[var(--text-primary)] font-semibold mb-1">
            {search ? "No scholars match your search" : "No scholar profiles yet"}
          </p>
          <p className="text-[var(--text-muted)] text-sm">
            {search ? "Try a different search term" : "Users with SCHOLAR role can create their profiles."}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div className="relative w-11 h-11 rounded-full overflow-hidden flex-shrink-0 border border-[var(--border-strong)]">
                    {scholar.photo ?? scholar.user.image ? (
                      <Image
                        src={(scholar.photo ?? scholar.user.image)!}
                ...
                <DropdownMenu.Root>
                  ...
                  <DropdownMenu.Portal>
                    ...
                    >
                      ...
                      >
                        <FiStar size={12} className={scholar.featured ? "text-[var(--accent)]" : ""} />
                        {scholar.featured ? "Remove Feature" : "Feature"}
                      </DropdownMenu.Item>

                      {/* Verify toggle */}
                      <DropdownMenu.Item
                        className="flex items-center gap-2 px-3 py-2 text-sm text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card-hover)] rounded-lg cursor-pointer outline-none transition-colors"
                        onClick={() => toggle(scholar.id, "verified", scholar.verified)}
                      >
                        <FiCheckCircle size={12} className={scholar.verified ? "text-emerald-400" : ""} />
                        {scholar.verified ? "Remove Verification" : "Verify"}
                      </DropdownMenu.Item>
                      ...
                      <DropdownMenu.Item
                        ...
                        onClick={() => remove(scholar.id, scholar.user.name)}
                      >
